...
static struct platform_device *snd_devices_topaz[] __initdata = {
//	&msm_iearpiece_device,
	&msm_imic_device,
	&msm_ispkr_stereo_device,
//	&msm_snddev_hdmi_stereo_rx_device,
...
void __init msm_snddev_init(void)
{
	...
	if (machine_is_msm8x60_surf() || machine_is_msm8x60_charm_surf()) {
		...
		for (i = 0; i < ARRAY_SIZE(snd_devices_topaz); i++)
			snd_devices_topaz[i]->id = dev_id++;

		platform_add_devices(snd_devices_topaz,
		ARRAY_SIZE(snd_devices_topaz));
	}
