...
extern int local_clk_enable_fixup(unsigned id);
/*
 * msm8x60 sets all local clocks' flags to _OFF_ in msm_clocks_8x60[] by default,
 * which means that all local clocks will to be turned off first when initialized.
 * But we want to keep some clocks on, such as display's related clocks. So here comes
 * the clock fixup workaround function, in order to clear the _OFF_ flags and set 
 * clock count.
 */
void __init msm_clock_fixup(struct clk *clock_tbl, unsigned num_clocks,
                    uint32_t *fixup_clk_ids, uint32_t fixup_clk_num)
{
    int i,j;
    struct clk * clk;

    for(i = 0; i < fixup_clk_num; i++)
    {
        for(j = 0; j < num_clocks; j++)
        {
            if(clock_tbl[j].id == fixup_clk_ids[i] && clock_tbl[j].remote_id == 0)
            {
                /* find the clock we need to keep on, clear the _OFF_ flag.*/
                clk = &clock_tbl[j];
                clk->flags = 0;
                clk->count = 1;
                clock_map_enabled[BIT_WORD(clk->id)] |= BIT_MASK(clk->id);
                local_clk_enable_fixup(clk->id);
                break;
            }
        }
    }
