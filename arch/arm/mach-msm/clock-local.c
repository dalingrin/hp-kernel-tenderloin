 ...
 * the clock fixup workaround function, in order to fixup the related stuff.This
 * function should be called _only_ by msm_clock_fixup().
 */
int __init local_clk_enable_fixup(unsigned id)
{
    struct clk_local *clk = &soc_clk_local_tbl[id];
    int rc = 0;
    ...
    if (!clk->count) {
        ...
        if (rc)
            goto err_vdd;
        if (clk->parent != C(NONE)) {
            rc = local_clk_enable_fixup(clk->parent);
            if (rc)
                goto err_par;
        }
        rc = local_src_enable(clk->current_freq->src);
