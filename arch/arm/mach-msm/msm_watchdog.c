...
#if defined(ADD_WATCHDOG_NOTE)
static int watchdog_state_get(void *data, u64 *val)
{
    *val = enable;
    return 0;
}

static ssize_t watchdog_state_set(void *data, u64 val)
{
    int rc = 0;
    ...
    is_enable =(unsigned long) val;

    if((enable == 1) && (is_enable == 1))
    {
        printk("%s : Already enable \n", __func__);
    }
    else if((enable != 1) && (is_enable == 1))
    {
      enable = 1;
      writel(1, WDT0_EN);
	  writel(1, WDT0_RST);
      ...
      printk("%s : Enable the watchdog reset \n", __func__);
    }
    else if((enable == 1) && (is_enable != 1))
    {
      enable = 0;
      writel(1, WDT0_RST);
      writel(0, WDT0_EN);
      printk("%s : Disable the watchdog reset \n", __func__);
    }
    else
    {
      enable = 0;
      printk("%s : Already disable the watch dog reset \n", __func__);
    }        
    return rc;
}
...
DEFINE_SIMPLE_ATTRIBUTE(watchdog_status_fops, watchdog_state_get,			watchdog_state_set, "%llu\n");
...
static int __init init_watchdog(void)
{
	...
	if (enable) {
        ...
        #if defined(ADD_WATCHDOG_NOTE)
        debugfs_base = debugfs_create_dir("watchdog", NULL);
        debugfs_create_file("status", S_IRUGO | S_IWUSR, debugfs_base,&enable, &watchdog_status_fops);
        #endif
