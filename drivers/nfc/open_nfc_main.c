...
int open_nfc_ioctl(struct inode *inode, struct file *filp,
                 unsigned int cmd, unsigned long arg)
{
   ...
   switch(cmd) {
     ...
     case OPEN_NFC_IOC_CONFIGURE:
        retval = open_nfc_custom_ioctl_configure(inode, filp, cmd, arg);
      break;

     case OPEN_NFC_IOC_CONNECT:
        retval = open_nfc_custom_ioctl_connect(inode, filp, cmd, arg);
      break;

     case OPEN_NFC_IOC_RESET:
        retval = open_nfc_custom_ioctl_reset(inode, filp, cmd, arg);
      break;
