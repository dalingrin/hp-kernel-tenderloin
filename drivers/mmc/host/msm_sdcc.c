...
//michael - disable power save for wifi debug
...
//michael - bug form Qualcomm driver board-msm8x60.c, the .dummy52_required should enable here, 
//          but we hard code to make it work
#if 0
		if ((host->plat->dummy52_required) &&
		    (host->dummy_52_state == DUMMY_52_STATE_SENT)) {
#else
		if (host->dummy_52_state == DUMMY_52_STATE_SENT) {
#endif
		...
		if ((status & (MCI_CMDSENT | MCI_CMDRESPEND | MCI_CMDCRCFAIL |
			...
			if (status & MCI_CMDTIMEOUT) {
				...
				pr_err("%s: Command timeout\n",
				       mmc_hostname(host->mmc));
#endif
				pr_err("%s: Command timeout,cmd->resp[0]=0x%x,cmd->arg=0x%x,cmd->opcode=%d\n",mmc_hostname(host->mmc),cmd->resp[0],cmd->arg,cmd->opcode);
				cmd->error = -ETIMEDOUT;
			} else if (status & MCI_CMDCRCFAIL &&
				cmd->flags & MMC_RSP_CRC) {
				pr_err("%s: Command CRC error,cmd->resp[0]=0x%x,cmd->arg=0x%x,cmd->opcode=%d\n",mmc_hostname(host->mmc),cmd->resp[0],cmd->arg,cmd->opcode);
				cmd->error = -EILSEQ;
			}
...
msmsdcc_request(struct mmc_host *mmc, struct mmc_request *mrq)
{
	...
	//HP zhanghong: Sep 21 15:25 CST 2010,begin
	//test for the system crash issue;
	if((mrq->cmd->opcode == MMC_WRITE_BLOCK 
	 || mrq->cmd->opcode == MMC_WRITE_MULTIPLE_BLOCK
	 || mrq->cmd->opcode == MMC_ERASE_GROUP_START) 
	 && mrq->cmd->arg == 0)
	{
		pr_err("%s: try to use command [%d] to write block 0, disable it\n", __func__,mrq->cmd->opcode);
		mdelay(10);
		BUG_ON(!NULL);
...
//	pr_info("%s: SDIO Wake up IRQ : %d, sdio_wlock(0x%x)\n", __func__, irq, host->sdio_wlock);
	spin_lock(&host->lock);
		...
		const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%d", &value)==1 && value) {
		printk("%s: call mmc_detect_change", __func__);
        mmc_detect_change(host->mmc, 0);
    }

	printk("%s: buf(%s) value(%d)\n", __func__, buf, value);
	return count;
}
...
msmsdcc_probe(struct platform_device *pdev)
{
	...
	if (plat->sdiowakeup_irq) {
		// 2010.10.14 HP Henry add: the interrupt will come before request_irq return sometimes
		// and cause exception for not initing sdio_wlock!!
		wake_lock_init(&host->sdio_wlock, WAKE_LOCK_SUSPEND, mmc_hostname(mmc));
		...
		if (ret) {
			...
			wake_lock_destroy(&host->sdio_wlock); // 2010.10.14 HP Henry add
			goto pio_irq_free;
		} else {
			mmc->pm_caps |= MMC_PM_WAKE_SDIO_IRQ;
			disable_irq(plat->sdiowakeup_irq);
			
/*	// 2010.10.14 HP Henry modify
