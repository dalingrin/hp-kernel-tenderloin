...
static int lcdc_on(struct platform_device *pdev)
{
	...
	/*
     * Check whether the lcdc is already running before set the clock rate and enable clock,
     * otherwise the screen will flash if there's displaying. This will also help to keep the clock
     * refcount balanced since we fixup the refcount of pixel_mdp_clk & pixel_lcdc_clk when
     * bootup.
     * FIXME: The best way of workaround should be using something like clk_is_enabled() here,
     * Unfortunately there's no such a standard API(refer linux/clk.h), so just skip set/enable
     * clock here when first boot, ugly though.
     */
    #if SKIP_BOOT_CLOCK_SET
    if (!first_boot)
    #endif
