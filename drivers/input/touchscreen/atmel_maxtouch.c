...
typedef struct
{
   ...
   u8 actv2idleto;   /*!< Active to idle power mode delay length in units of 0.2s*/
...
#define mxt_debug(level, ...) \
    ...
    static int mxt_write_block(struct i2c_client *client, u16 addr, u16 length,
                   u8 *value);
    static int  write_power_config(struct mxt_data *mxt, 
                   gen_powerconfig_t7_config_t * pCFG);
...
static int  write_power_config(struct mxt_data *mxt, gen_powerconfig_t7_config_t * pCFG)
{
    return  mxt_write_block(mxt->client, MXT_BASE_ADDR(MXT_GEN_POWERCONFIG_T7, mxt), 3 ,
                   (u8 *)pCFG);
		   ...
		   u8 *value)
{
	...
	if (mxt != NULL) {
		if ((mxt->last_read_addr == addr) &&
			...
			if  ((ret = i2c_master_recv(client, value, length)) == length)
				return length;
			else
			{
                pr_err("atmel mxt_read_block %x\r\n",ret);
				return -EIO;
			}
		} else {
...
static void mxt_worker(struct work_struct *work)
{
	...
	message = NULL;
	mxt = container_of(work, struct mxt_data, dwork);
	disable_irq(mxt->irq);
	client = mxt->client;
	...
	if (mxt->read_chg() == 0){
		queue_work(mxt->touch_wq, &mxt->dwork);
	}
...
static irqreturn_t mxt_irq_handler(int irq, void *_mxt)
{
	...
	mxt->irq_counter++;
	if (mxt->valid_interrupt() && !(mxt->isSuspend)) {
				  ...
				  u8 *id_block_data)
{
    ...
    while(trytime < 6)
    {
        ...
        trytime++;
        /*
        If the Wake line is connected to the SCL pin, the mXT1386 will send a NACK on the first attempt to 
        address it; the host then retry 25 ms later
        The mXT1386 is ready to accept I2C-compatible communications 25ms after the WAKE line is asserted.
        */
        mdelay(25);
    ...
    if(trytime)
    {
        pr_err(" NO CFG file in touchscreen sensor  \n\r");
        //return -EIO;
    }
                ...
                const char *buf, size_t size)
{
    ...
    printk("%s : is_enable = %d, size = %d isInterrupt_Enabled= %d\n", __func__, (unsigned int)is_enable, size , mxt->isInterrupt_Enabled);

    if(is_enable && mxt->isInterrupt_Enabled)
        return size;
    ...
    if(is_enable && !(mxt->isInterrupt_Enabled))
    {
        ...
        // fix TD103:Touch panel no response after read SNR raw data
        /* Make sure we just didn't miss a interrupt. */
        if (mxt->read_chg() == 0){
            queue_work(mxt->touch_wq, &mxt->dwork);
        }
        ...
        mxt->isInterrupt_Enabled = 1;
    }
    
    if(!is_enable && mxt->isInterrupt_Enabled)
    {
        free_irq(mxt->irq, mxt);
        mxt->isInterrupt_Enabled = 0;
    }
			       ...
			       const struct i2c_device_id *id)
{
    ...
    if(pdata->irq_gpio != NULL)
    {
        client->irq = MSM_GPIO_TO_INT(pdata->irq_gpio());
        mxt->irq = client->irq;
    }
	...
	mxt->nontouch_msg_only = 1;
	INIT_WORK(&mxt->dwork, mxt_worker);
    mxt->touch_wq = create_workqueue("touch_irq_wq");
    ...
    error = device_create_file(&client->dev, &dev_attr_interrupt_enable);
    if (error < 0) {
...
static int mxt_resume(struct i2c_client *client)
{
    ...
    pr_err("atmel mxt_resume\n");

    // Wake up touch sensor first
    // poll devices 6 times
    while(trytime < 6)
    {
        /* Read Device info to check if chip is valid */
    ...
    /* Set Idle Acquisition Interval to 50ms */
    cfg.idleacqint = 50;
    /* Set Active Acquisition Interval to free-run mode. */
    cfg.actvacqint = 255;
    /* Set Active to Idle Timeout to 5 s (one unit = 200ms). */
    cfg.actv2idleto = 25;
