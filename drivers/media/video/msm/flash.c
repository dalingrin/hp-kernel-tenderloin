...
//Head files needed for Opal camera flash related operations
	...
	unsigned led_state)
{
	...
	switch (led_state) {
	case MSM_CAMERA_LED_OFF:
    //Correct flash shut down sequence, should shutdown flash device before revert mode. Bob Zhu--0130
		rc=lm8502_flashtorch_start_export(0);
		if(rc<0)
		{
			pr_err("%s: led_state = %d stop flash failed\n", __func__, led_state);
		}
		rc=lm8502_flash_enable_export(0);//0 -Off 1-Flash 2-Torch
		if(rc<0)
		{
			pr_err("%s: led_state = %d reset mode failed\n", __func__, led_state);
		}		
		break;
	case MSM_CAMERA_LED_LOW:
		rc=lm8502_flash_enable_export(2);
		if(rc<0)
		{
			pr_err("%s: led_state = %d set torch mode failed\n", __func__, led_state);
		}
		rc=lm8502_flashtorch_start_export(1);
		if(rc<0)
		{
			pr_err("%s: led_state = %d start torch failed\n", __func__, led_state);
		}
		break;
	case MSM_CAMERA_LED_HIGH:
		rc=lm8502_flash_enable_export(1);
		if(rc<0)
		{
			pr_err("%s: led_state = %d set flash mode failed\n", __func__, led_state);
		}
		rc=lm8502_flashtorch_start_export(1);
		if(rc<0)
		{
			pr_err("%s: led_state = %d start flash failed\n", __func__, led_state);
		}
		break;
	default:
		rc = -EFAULT;
		pr_err("%s: led_state = %d unsupported led state\n", __func__, led_state);
	    break;
	}
#endif
