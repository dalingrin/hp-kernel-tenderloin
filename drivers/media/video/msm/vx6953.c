...
static int32_t vx6953_sensor_setting(int update_type, int rt)
{
	...
	if (vx6953_ctrl->sensor_type != VX6953_STM5M0EDOF_CUT_2) {
		...
		if (rt == RES_PREVIEW || rt == RES_CAPTURE) {
			...
			//Temporarily change value of settle_cnt to 14 to resolve 5M camera can't open on some boards issue.			
...
{
    //As Opal 5M camera won't use reset pin, so discard related codes. Bob Zhu
