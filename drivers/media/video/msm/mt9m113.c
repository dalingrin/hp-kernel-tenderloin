...
static int32_t mt9m113_reg_init(void)
{
  ...
  if(rc<0)
  	  return rc;
  //Move last part of initialize list here, and also use polling method to make sure MCU refresh finished before leave initialize.	
  //Polling below can resolve "black screen issue we met" --Bob Zhu 0129
   do
    {                               
...
static int32_t mt9m113_set_sensor_mode(int mode)
{
  ...
  if (config_csi == 0) {
	...
	case SENSOR_PREVIEW_MODE:
	  ...
	  //Would use another mechanism to make sure sensor data is ready(read related sensor registers instead of delay)        	                	
      //mdelay(20);
		break;
...
//Codes below to ensure camera sensor data are ready before leaving configuration. 0124 Bob Zhu
 do
   {                               
    rc = mt9m113_i2c_write(mt9m113_client->addr, 0x098C, 0xA103, WORD_LEN); 
    if (rc < 0) return rc;
...
static int mt9m113_sensor_init_probe(const struct msm_camera_sensor_info *data)
{
	uint16_t model_id = 0;
	int rc = 0;

	CDBG("mt9m113_sensor_init_probe E\n");

  //Here we should do operations for camera power up as well as reset, we have moved that to board file. 
	...
	if (model_id != MT9M113_MODEL_ID) {
		printk("mt9m113 new model_id = 0x%x\n", model_id);
		rc = -ENODEV;
		goto init_probe_fail;
	}
	...
	if (rc < 0)
	  {
		 printk("%s mt9m113_reg_init failed\n", __func__);
		 goto init_probe_fail;
	  }
		
	CDBG("mt9m113_sensor_init_probe X\n");
	return rc;
...
int mt9m113_sensor_init(const struct msm_camera_sensor_info *data)
{
  ...
  config_csi = 0;
	...
	rc = mt9m113_sensor_init_probe(data);
	if (rc < 0) {
				...
				struct msm_sensor_ctrl *s)
{
  ...
  CDBG("mt9m113_sensor_probe E\n");
	...
	rc = mt9m113_sensor_init_probe(info);
	if (rc < 0)
		goto probe_fail;
