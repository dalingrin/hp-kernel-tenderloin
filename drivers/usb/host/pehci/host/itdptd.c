...
{
	unsigned int i = 0;
	u16 skipmap = 0; //songhao for removing warning
	struct ehci_sitd *sitd;// *current_sitd; //songhao for removing wanring
	struct ehci_itd *itd;
	//struct urb *urb = NULL; //songhao for removing warning
...
{
	unsigned int i = 0;
	//int retval = 0; //songhao for removing warning
	struct ehci_qh *qhead;
	//struct list_head *sitd_itd_list, *position, *temp; //songhao removing warning
	struct ehci_sitd *sitd;// *current_sitd; //songhao removing warning
	struct ehci_itd *itd;
	u16 skipmap = 0;//songhao for removing warning
		...
		if (urb->dev->speed == USB_SPEED_FULL) {	/*for FULL SPEED */
			...
			if(1){
				if (phcd_submit_iso(hcd, urb,
					(unsigned long *) &status) == 0) {//songhao for removing warning
			...
			if (hcd->periodic_sched <=
				...
				if (phcd_submit_iso(hcd, urb, (unsigned long *) &status)
					== 0) {
		...
		struct urb *urb, unsigned long *status)
{
	...
	unsigned long temp;
	//unsigned long flags, packets; //songhao for removing warning
	unsigned long  packets;
