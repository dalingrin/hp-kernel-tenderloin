...
#include "itdptd.c"
#endif /* CONFIG_ISO_SUPPORT */
struct class *usb_hcd_class;
static int scratch_mismatch = 0;

static void pehci_hcd_suspend(struct isp1763_dev *dev);
static void pehci_hcd_resume(struct isp1763_dev *dev);
static int  __devexit pehci_hcd_remove(struct platform_device *dev);
static void pehci_hcd_remotewakeup(struct isp1763_dev *dev);
static int pehci_hcd_init(struct isp1763_dev *tmp_1763_dev, isp1763_id * ids);
static struct isp1763_driver pehci_hcd_util_driver;
...
{
	u32 donemap = 0, skipmap = 0; /*ormask = 0, */
	u32 pendingmap = 0;
...
void
pehci_hcd_remote_wakeup_irq(struct usb_hcd *usb_hcd)
{
	phci_hcd *pehci_hcd;
	...
	mdelay(1);
	printk("#### %s ___int_reg 0x%08x\n", __func__, (unsigned int)dev->int_reg);
	isp1763_reg_write16(dev, HC_INTERRUPT_REG, dev->int_reg);//clear the int

	temp = isp1763_reg_read32(dev, HC_USBSTS_REG, 0);
	printk("__USBSTST %x\n", temp);
	isp1763_reg_write32(dev, HC_USBSTS_REG, temp);
...
#else
irqreturn_t
pehci_hcd_irq(struct usb_hcd *hcd)
#endif
{
	...
	usbsts = isp1763_reg_read32(dev, HC_USBSTS_REG, 0);
	/*
	 * port status register. bit4 is
	 * OPR register change
	 */
	if ( (usbsts & 0x4) && (intr & 0x50)  )
	{
		//spin_lock(&pehci_hcd->lock);
		portsc = isp1763_reg_read32(dev, HC_PORTSC1_REG, 0);
		//spin_unlock(&pehci_hcd->lock);
		if (portsc & 0x4)
			pehci_hcd_remote_wakeup_irq(usb_hcd);
		resume = 1;
	}
	else
	{
		...
		usbsts = isp1763_reg_read32(dev, HC_USBSTS_REG, 0);
		isp1763_reg_write32(dev, HC_USBSTS_REG, usbsts);
		//spin_unlock(&pehci_hcd->lock);
	}
		 ...
		 u16 wIndex, char *buf,	u16 wLength)
{
	...
	printk("%s: request 0x%08x, wValuse:0x%08x, wIndex:%d \n",__func__, typeReq,wValue,wIndex);
	...
	case GetPortStatus:
		...
		temp = isp1763_reg_read32(hcd->dev, hcd->regs.ports[wIndex],
					  temp);
		printk("GetPortStatus: root port %d (0x%08X) status:0x%x\n", wIndex, hcd->regs.ports[wIndex], temp);
	...
	case SetPortFeature:
		...
		temp = isp1763_reg_read32(hcd->dev, hcd->regs.ports[wIndex],
					  temp);
		printk("SetPortFeature:PortSc Val 0x%08x\n", temp);
		if (temp & PORT_OWNER) {
			break;
		}
		switch (wValue)	{
...
static const struct hc_driver pehci_driver = {
	...
	/*
	 * bus suspend and resume.(Should has these two functions otherwise, suspend/resume can not successfully)
	 */
...
static ssize_t scratch_mismatch_show(struct device *dev,
					struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", scratch_mismatch);
}
static DEVICE_ATTR(scratch_mismatch, 0644, scratch_mismatch_show, NULL);


#ifdef  DUMP_REGISTER_INFO

char dumpbuffer[4096];

void isp1763_dump_all_regs_to_buff(void)
{
	char tmpbuf[200];
	u32 temp;
	...
	//USBCMD
	temp = isp1763_reg_read32(isp1763_hcd, HC_USBCMD_REG, 0);
	sprintf(tmpbuf, " USBCMD:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//USBSTS
	temp = isp1763_reg_read32(isp1763_hcd, HC_USBSTS_REG, 0);
	sprintf(tmpbuf, " USBSTS:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//USBINTR
	temp = isp1763_reg_read32(isp1763_hcd, HC_INTERRUPT_REG_EHCI, 0);
	sprintf(tmpbuf, " USBINTR:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//FRINDEX
	temp = isp1763_reg_read16(isp1763_hcd, HC_FRINDEX_REG, 0);
	sprintf(tmpbuf, " FRINDEX:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//CONFIGFLAG
	temp = isp1763_reg_read32(isp1763_hcd, HC_CONFIGFLAG_REG, 0);
	sprintf(tmpbuf, " CONFLGFLAG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//PORTSC1
	temp = isp1763_reg_read32(isp1763_hcd, HC_PORTSC1_REG, 0);
	sprintf(tmpbuf, " PORTSC1:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HW Mode Control
	temp = isp1763_reg_read16(isp1763_hcd, HC_HW_MODE_REG, 0);
	sprintf(tmpbuf, " HW Mode Control:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_CHIP_ID_REG
	temp = isp1763_reg_read32(isp1763_hcd, HC_CHIP_ID_REG, 0);
	sprintf(tmpbuf, " HC_CHIP_ID_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_SCRATCH_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_SCRATCH_REG, 0);
	sprintf(tmpbuf, " HC_SCRATCH_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//SW Reset HC_RESET_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_RESET_REG, 0);
	sprintf(tmpbuf, " SW Reset HC_RESET_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_HWMODECTRL_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_HWMODECTRL_REG, 0);
	sprintf(tmpbuf, " HC_HWMODECTRL_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HcBufferStatus
	temp = isp1763_reg_read16(isp1763_hcd, HC_BUFFER_STATUS_REG, 0);
	sprintf(tmpbuf, " HcBufferStatus:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HcDMAConfiguration
	temp = isp1763_reg_read32(isp1763_hcd, HC_DMACONFIG_REG, 0);
	sprintf(tmpbuf, " HcDMAConfiguration:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);


	//HC_UNLOCK_DEVICE
	temp = isp1763_reg_read16(isp1763_hcd, HC_UNLOCK_DEVICE, 0);
	sprintf(tmpbuf, " HC_UNLOCK_DEVICE:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);


	//HcInterrupt
	temp = isp1763_reg_read16(isp1763_hcd, HC_INTERRUPT_REG, 0);
	sprintf(tmpbuf, " HcInterrupt:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);


	//HcInterruptEnable
	temp = isp1763_reg_read16(isp1763_hcd, HC_INTENABLE_REG, 0);
	sprintf(tmpbuf, " HcInterruptEnable:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//ISO IRQ MASK OR
	temp = isp1763_reg_read16(isp1763_hcd, HC_ISO_IRQ_MASK_OR_REG, 0);
	sprintf(tmpbuf, " ISO IRQ MASK OR:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//INT IRQ MASK OR
	temp = isp1763_reg_read16(isp1763_hcd, HC_INT_IRQ_MASK_OR_REG, 0);
	sprintf(tmpbuf, " INT IRQ MASK OR:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//ATL IRQ MASK OR
	temp = isp1763_reg_read16(isp1763_hcd, HC_ATL_IRQ_MASK_OR_REG, 0);
	sprintf(tmpbuf, " ATL IRQ MASK OR:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//ISO IRQ MASK AND
	temp = isp1763_reg_read16(isp1763_hcd, HC_ISO_IRQ_MASK_AND_REG, 0);
	sprintf(tmpbuf, " ISO IRQ MASK AND:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//INT IRQ MASK AND
	temp = isp1763_reg_read16(isp1763_hcd, HC_INT_IRQ_MASK_AND_REG, 0);
	sprintf(tmpbuf, " INT IRQ MASK AND:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//ATL IRQ MASK AND
	temp = isp1763_reg_read16(isp1763_hcd, HC_ATL_IRQ_MASK_AND_REG, 0);
	sprintf(tmpbuf, " ATL IRQ MASK AND:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_POWER_DOWN_CONTROL_REG
	temp = isp1763_reg_read32(isp1763_hcd, HC_POWER_DOWN_CONTROL_REG, 0);
	sprintf(tmpbuf, " HC_POWER_DOWN_CONTROL_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_SPARAMS_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_SPARAMS_REG, 0);
	sprintf(tmpbuf, " HC_SPARAMS_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_CPARAMS_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_CPARAMS_REG, 0);
	sprintf(tmpbuf, " HC_CPARAMS_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	/*ISO Transfer Registers */
	//HC_ISO_PTD_DONEMAP_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_ISO_PTD_DONEMAP_REG, 0);
	sprintf(tmpbuf, " HC_ISO_PTD_DONEMAP_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_ISO_PTD_SKIPMAP_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_ISO_PTD_SKIPMAP_REG, 0);
	sprintf(tmpbuf, " HC_ISO_PTD_SKIPMAP_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	...
	temp = isp1763_reg_read16(isp1763_hcd, HC_INT_PTD_DONEMAP_REG, 0);
	sprintf(tmpbuf, " HC_INT_PTD_DONEMAP_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_INT_PTD_SKIPMAP_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_INT_PTD_SKIPMAP_REG, 0);
	sprintf(tmpbuf, " HC_INT_PTD_SKIPMAP_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//HC_INT_PTD_LASTPTD_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_INT_PTD_LASTPTD_REG, 0);
	sprintf(tmpbuf, " HC_INT_PTD_LASTPTD_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	/*ATL Transfer Registers */
	//HC_ATL_PTD_DONEMAP_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_ATL_PTD_DONEMAP_REG, 0);
	sprintf(tmpbuf, " HC_ATL_PTD_DONEMAP_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//HC_ATL_PTD_SKIPMAP_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_ATL_PTD_SKIPMAP_REG, 0);
	sprintf(tmpbuf, " HC_ATL_PTD_SKIPMAP_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//HC_ATL_PTD_LASTPTD_REG
	temp = isp1763_reg_read16(isp1763_hcd, HC_ATL_PTD_LASTPTD_REG, 0);
	sprintf(tmpbuf, " HC_ATL_PTD_LASTPTD_REG:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	///////////////////////////////////
	//Initialization registers
	//00 Address
	temp = isp1763_reg_read8(isp1763_hcd, 0x00, 0);
	sprintf(tmpbuf, " Peripheral controller Address :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//0Ch Mode
	temp = isp1763_reg_read16(isp1763_hcd, 0x0c, 0);
	sprintf(tmpbuf, " Peripheral controller Mode :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//10h Interrupt Configuration
	temp = isp1763_reg_read8(isp1763_hcd, 0x10, 0);
	sprintf(tmpbuf, " Peripheral controller Interrupt Configuration :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//14h DcInterruptEnable
	temp = isp1763_reg_read32(isp1763_hcd, 0x14, 0);
	sprintf(tmpbuf, " Peripheral controller  DcInterruptEnable :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);


	//2Ch Endpoint Index
	temp = isp1763_reg_read8(isp1763_hcd, 0x2C, 0);
	sprintf(tmpbuf, " Peripheral controller  Endpoint Index :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//28h Control Function
	temp = isp1763_reg_read8(isp1763_hcd, 0x28, 0);
	sprintf(tmpbuf, " Peripheral controller  Control Function :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//20h Data Port
	temp = isp1763_reg_read16(isp1763_hcd, 0x20, 0);
	sprintf(tmpbuf, " Peripheral controller  Data Port :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//1C Buffer Length
	temp = isp1763_reg_read16(isp1763_hcd, 0x1c, 0);
	sprintf(tmpbuf, " Peripheral controller  Buffer Length :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//1E DcBufferStatus
	temp = isp1763_reg_read8(isp1763_hcd, 0x1E, 0);
	sprintf(tmpbuf, " Peripheral controller  DcBufferStatus :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);


	//04h Endpoint MaxPacketSize
	temp = isp1763_reg_read16(isp1763_hcd, 0x04, 0);
	sprintf(tmpbuf, " Peripheral controller  Endpoint MaxPacketSize :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//08h Endpoint Type
	temp = isp1763_reg_read16(isp1763_hcd, 0x08, 0);
	sprintf(tmpbuf, " Peripheral controller  Endpoint Type :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	///////////////////////////////////
	//DMA registers
	//30h DMA command
	temp = isp1763_reg_read8(isp1763_hcd, 0x30, 0);
	sprintf(tmpbuf, " Peripheral controller   DMA command :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//34h DMA Transfer Counter
	temp = isp1763_reg_read32(isp1763_hcd, 0x34, 0);
	sprintf(tmpbuf, " Peripheral controller   DMA Transfer Counter :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//38h DcDMAConfiguration
	temp = isp1763_reg_read16(isp1763_hcd, 0x38, 0);
	sprintf(tmpbuf, " Peripheral controller   DcDMAConfiguration :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//3Ch DMA Hardware
	temp = isp1763_reg_read8(isp1763_hcd, 0x3c, 0);
	sprintf(tmpbuf, " Peripheral controller   DMA Hardware :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//50h DMA Interrupt Reason
	temp = isp1763_reg_read16(isp1763_hcd, 0x50, 0);
	sprintf(tmpbuf, " Peripheral controller   DMA Interrupt Reason :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//54h DMA Interrupt Enable
	temp = isp1763_reg_read16(isp1763_hcd, 0x54, 0);
	sprintf(tmpbuf, " Peripheral controller   DMA Interrupt Enable:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//58h DMA Endpoint
	temp = isp1763_reg_read8(isp1763_hcd, 0x58, 0);
	sprintf(tmpbuf, " Peripheral controller   DMA Endpoint:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//64h DMA Burst Counter
	temp = isp1763_reg_read16(isp1763_hcd, 0x64, 0);
	sprintf(tmpbuf, " Peripheral controller   DMA Burst Counter:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	///////////////////////////////////
	//18h DcInterrupt
	temp = isp1763_reg_read32(isp1763_hcd, 0x18, 0);
	sprintf(tmpbuf, " Peripheral controller   DcInterrupt:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//70h Chip ID
	temp = isp1763_reg_read32(isp1763_hcd, 0x18, 0);
	sprintf(tmpbuf, " Peripheral controller   Chip ID:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//74h Frame Number
	temp = isp1763_reg_read16(isp1763_hcd, 0x74, 0);
	sprintf(tmpbuf, " Peripheral controller   Frame Number:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//78h Scratch
	temp = isp1763_reg_read16(isp1763_hcd, 0x78, 0);
	sprintf(tmpbuf, " Peripheral controller  Scratch:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//7Ch Unlock Device
	temp = isp1763_reg_read16(isp1763_hcd, 0x7c, 0);
	sprintf(tmpbuf, " Peripheral controller Unlock Device:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//80h Interrupt Pulse Width
	temp = isp1763_reg_read8(isp1763_hcd, 0x80, 0);
	sprintf(tmpbuf, " Peripheral controller Interrupt Pulse Width:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//84h TestMode
	temp = isp1763_reg_read8(isp1763_hcd, 0x84, 0);
	sprintf(tmpbuf, " Peripheral controller TestMode:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//OTG control register
	//E4h Set
	temp = isp1763_reg_read16(isp1763_hcd, 0xe4, 0);
	sprintf(tmpbuf, " OTG Control  Set:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
	//E6h Clear
	temp = isp1763_reg_read16(isp1763_hcd, 0xe6, 0);
	sprintf(tmpbuf, " OTG Control Clear:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//E8h OTG interrupt Source
	temp = isp1763_reg_read16(isp1763_hcd, 0xe8, 0);
	sprintf(tmpbuf, " OTG interrupt Souce:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//ECh OTG interrupt Latch Set
	temp = isp1763_reg_read16(isp1763_hcd, 0xec, 0);
	sprintf(tmpbuf, " OTG interrupt Latch Set :%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//EEh OTG interrupt Latch Clear
	temp = isp1763_reg_read16(isp1763_hcd, 0xee, 0);
	sprintf(tmpbuf, " OTG interrupt Latch Clear:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//F0h OTG interrupt Enable Fall Set
	temp = isp1763_reg_read16(isp1763_hcd, 0xF0, 0);
	sprintf(tmpbuf, " OTG interrupt Enable Fail:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//F2h OTG interrupt Enable Fall Clear
	temp = isp1763_reg_read16(isp1763_hcd, 0xF2, 0);
	sprintf(tmpbuf, " OTG interrupt Enable Fail:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);


	//F4h OTG interrupt Enable Rise Set
	temp = isp1763_reg_read16(isp1763_hcd, 0xF4, 0);
	sprintf(tmpbuf, " OTG interrupt Enable Rise Set:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//F6h OTG interrupt Enable  Rise Clear
	temp = isp1763_reg_read16(isp1763_hcd, 0xF6, 0);
	sprintf(tmpbuf, " OTG interrupt Enable Rise Clear:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//////////////////////////
	//OTG Timer register
	//Time Low Word Set F8h
	temp = isp1763_reg_read16(isp1763_hcd, 0xF8, 0);
	sprintf(tmpbuf, " OTG interrupt Time Low Word Set:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//Time Low Word Clear FAh
	temp = isp1763_reg_read16(isp1763_hcd, 0xFa, 0);
	sprintf(tmpbuf, " OTG interrupt Time Low Word Clear:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//Time High Word Set FCh
	temp = isp1763_reg_read16(isp1763_hcd, 0xFC, 0);
	sprintf(tmpbuf, " OTG interrupt Time High Word Set:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);

	//Time High Word Clear FEh
	temp = isp1763_reg_read16(isp1763_hcd, 0xFE, 0);
	sprintf(tmpbuf, " OTG interrupt Time High Word Clear:%x \n", temp);
	strcat(dumpbuffer,tmpbuf);
}

void isp1763_dump_all_register_to_buf_disable_irq(void)
{
	if ( !isp1763_hcd )
	{	printk("#### isp1763 is null");
		return 0;
	}
	disable_irq(isp1763_hcd->irq);
	isp1763_dump_all_regs_to_buff();
	enable_irq(isp1763_hcd->irq);
}

void isp1763_dump_regs_to_console(void)
{
	if ( !isp1763_hcd )
	{	printk("#### isp1763 is null");
		return 0;
	...
	isp1763_dump_all_register_to_buf_disable_irq();
...
static DEVICE_ATTR(isp1763_register, 0644, register_show, NULL);

#endif

/*probe platform device */
static int __devinit
pehci_hcd_probe(struct platform_device *pdev)
/*probe the PCI host*/
{
	struct isp1763_dev *tmp_1763_dev = pdev->dev.platform_data;
	int status = 0;
	struct device *dev;

	status = device_create_file(tmp_1763_dev->dev, &dev_attr_scratch_mismatch);
	if (status < 0)
		printk(KERN_ERR"%s: Failed to create scratch_mismatch dev attr (%d)\n", __func__, status);

	#ifdef DUMP_REGISTER_INFO
	status = device_create_file(tmp_1763_dev->dev, &dev_attr_isp1763_register);
	if (status < 0)
		printk(KERN_ERR"%s: Failed to create isp1763_register dev attr (%d)\n", __func__, status);
	#endif

	if ( (status = pehci_hcd_init(tmp_1763_dev, pehci_hcd_util_driver.id))!=0 )
	{
		goto err3;
	}
	else
	{
		status = register_chrdev(USB_HCD_MAJOR, USB_HCD_MODULE_NAME, &usb_hcddev_fops);
		if(status)
		{
			printk("register_chrdev USB_HCD_MAJOR error\n");
			status = -1;
			goto err3;
		}
		usb_hcd_class = class_create(THIS_MODULE, "usb_hcd");
		if (IS_ERR(usb_hcd_class))
		{
			printk("Failed in create class usb_hcd\n");
			status = -1;
			goto err2;
		}
		dev = device_create(usb_hcd_class,NULL, MKDEV(USB_HCD_MAJOR,0),NULL,"isp1763hcd" );
		if (!dev)
		{
	...
	wake_lock_init(&wlock, WAKE_LOCK_SUSPEND, "isp1763");
	return 0;
err1:
	class_destroy(usb_hcd_class);
err2:
...
/* platform driver glue; this is a "new style" platform driver module */
static struct platform_driver pehci_hcd_platform_driver = {
	.probe = pehci_hcd_probe,
	.remove = __exit_p(pehci_hcd_remove),
	.driver = {
		.name  = hcd_name,
		.owner = THIS_MODULE,
	},
...
int
pehci_hcd_init(struct isp1763_dev *tmp_1763_dev, isp1763_id * ids)
{
	struct usb_hcd *usb_hcd;
	...
	pehci_hcd->next_statechange = jiffies;

	/*lets keep our host here */
	tmp_1763_dev->driver_data = usb_hcd;
	tmp_1763_dev->driver = &pehci_hcd_util_driver;
	/* modify for HCD testing */
#ifdef CONFIG_USB_ISP1763A_EHCI_TEST_SUPPORT
	No_Data_Phase =	0;
	No_Status_Phase	= 0;
	#endif
...
static int isp1763a_suspend(struct usb_hcd *usb_hcd)
{
	...
	if (time_before(jiffies, pehci_hcd->next_statechange))
		msleep(5);
	...
	hcdpowerdown = 1;
	usb_hcd->state = HC_STATE_SUSPENDED;
	 enable_irq_wake(pehci_hcd->dev->irq);
	pehci_hcd->next_statechange = jiffies + msecs_to_jiffies(5);

	return 0;
}

/*
Bus resume function for usb host
*/
static int isp1763a_resume(struct usb_hcd *usb_hcd)
{
	phci_hcd *pehci_hcd = NULL;
	...
	disable_irq_wake(pehci_hcd->dev->irq);
	...
	//printk("interrupt enable register %x\n", temp);
	temp = isp1763_reg_read32(dev, HC_USBSTS_REG, 0x0);
	isp1763_reg_write32(dev, HC_USBSTS_REG, temp); //0x90
	isp1763_reg_write32(dev, HC_INTERRUPT_REG_EHCI, 0x0); //0x94
	temp = isp1763_reg_read32(dev, HC_PORTSC1_REG, 0);
	printk("port status %x\n ", temp);
	temp = isp1763_reg_read16(dev, HC_USBCMD_REG, 0);
	...
	for (temp = 0; temp <100; temp++)
	{
		i = isp1763_reg_read32(dev, HC_POWER_DOWN_CONTROL_REG, 0xffff08a0);
		if (i ==0xffff08a0)
			break;
		mdelay(2);
	}

	printk("POWER_DOWN_CONTROL %x temp = %x\n",i, temp);
	temp = isp1763_reg_read32(dev, HC_PORTSC1_REG, 0);
	if (temp & PORT_SUSPEND)
	{
		printk("^^^Port suspended,try to resume PORTSC1=0x%x\n",temp);
		/* Why suspend it first?*/
		//temp = isp1763_reg_read32(dev, HC_PORTSC1_REG, 0);
		//temp |= 0x80;
		//isp1763_reg_write32(dev, HC_PORTSC1_REG, temp);
		//mdelay(50);
...
//	mdelay(10);
	...
	pehci_hcd->next_statechange = jiffies + msecs_to_jiffies(250);
	hcdpowerdown = 0;
	usb_hcd->state = HC_STATE_RUNNING;
...
/*
The really function to do the resume
*/
void
...
//	struct pci_dev *pcidev = dev->pcidev;
	...
	//this state should be after the irq is disabled
	usb_hcd->state = HC_STATE_SUSPENDED;
	temp = isp1763_reg_read16(dev, HC_USBCMD_REG, 0);
	temp &= ~0x01;		/* stop the controller first */
	isp1763_reg_write16(dev, HC_USBCMD_REG, temp);
...
pehci_hcd_remove(struct platform_device *pdev)
{
	struct isp1763_dev *tmp_1763_dev = pdev->dev.platform_data;
	struct usb_hcd *usb_hcd;
...
//	isp1763_free_irq(isp1763_dev,usb_hcd);
	...
	device_remove_file(tmp_1763_dev->dev, &dev_attr_scratch_mismatch);

	device_destroy(usb_hcd_class,MKDEV(USB_HCD_MAJOR,0));
	class_destroy(usb_hcd_class);
...
pehci_module_init( void )
{
	...
	phci_hcd_mem_init();
	/*register driver */
	if((result = platform_driver_register(&pehci_hcd_platform_driver)) < 0) {
		printk(KERN_NOTICE "pehci_module_init platform_driver_register() fail: result :0x%08x\n", result);
		return result;
	} else {
		printk(KERN_NOTICE "pehci_module_init platform_driver_register() success: result :0x%08x\n", result);
	}
...
#endif
	platform_driver_unregister(&pehci_hcd_platform_driver);
