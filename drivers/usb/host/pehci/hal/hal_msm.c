...
int register_ehci_hcd_driver( void );
...
static struct platform_device isp1763_hcd = {
	.name          = hcd_name,
	.dev           = {
		.platform_data = &(isp1763_loc_dev[ISP1763_HC]),
	}
};
...
isp1763_module_init(void)
{
	...
	if((result = platform_driver_register(&isp1763_usb_driver)) < 0) {
		printk(KERN_NOTICE "driver_register() fail: result :0x%08x\n", result);
		return result;
	} else {
		printk(KERN_NOTICE "driver_register() success: result :0x%08x\n", result);
	}
...
{
	u32 w = 0;
	// isp1763_reg_write16(dev, HC_MEM_READ_REG, 0x0400); // Song Hao: Disable it based on Chip Spec
	w = isp1763_reg_read16(dev, HC_DATA_REG, w);
...
isp1763_probe(struct platform_device *pdev)
{
	...
	pr_info("%s: ioremap done at: %x \n", __func__, (u32)loc_dev->baseaddress);
	...
	if ( platform_device_register(&isp1763_hcd) )
	{
		status = -EBUSY;
		goto free_regs;
	}
