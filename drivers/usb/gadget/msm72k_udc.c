...
static void usb_chg_detect(struct work_struct *w)
{
	...
	otg_io_write_with_reset(ui->xceiv, 0x0f, 0x34);
	msleep(10);
	otg_io_write_with_reset(ui->xceiv, 0x4d, 0x04);
	otg_io_write_with_reset(ui->xceiv, 0x06, 0x0c);
	msleep(20);

	if ((readl(USB_PORTSC) & PORTSC_LS) & (1 << 11)) { //D+ : high
		if ((readl(USB_PORTSC) & PORTSC_LS) & (1 << 10)) { //D+ : high, D- : high
			otg_io_write_with_reset(ui->xceiv, 0x45, 0x04);
			otg_io_write_with_reset(ui->xceiv, 0x2, 0x0b); //pull-down D+
			msleep(100);
			printk("UDC-CHG (2-1): %s (%d) : (readl(USB_PORTSC) & PORTSC_LS) = 0x%x\n", __func__, __LINE__, (readl(USB_PORTSC) & PORTSC_LS));
			if ((readl(USB_PORTSC) & PORTSC_LS) & (1 << 10)) { //D+ : high, D- : high
				...
				printk("UDC-CHG (2-1-1-2): %s (%d) : topaz Adaptor(2000mA)!\n", __func__, __LINE__);
				temp = USB_CHG_TYPE__WALLCHARGER;
				atomic_set(&otg->chg_type, temp);
			...
			printk("UDC-CHG (2-1-2): %s (%d) : Invalid Adaptor(100mA)!\n", __func__, __LINE__);
			temp = USB_CHG_TYPE__WALLCHARGER;
			atomic_set(&otg->chg_type, temp);
			maxpower = 100;
		...
		if ((readl(USB_PORTSC) & PORTSC_LS) & (1 << 10)) { //D+ : low, D- : high
			printk("UDC-CHG (2-2): %s (%d) : Invalid Adaptor(100mA)!\n", __func__, __LINE__);
			temp = USB_CHG_TYPE__WALLCHARGER;
			atomic_set(&otg->chg_type, temp);
			maxpower = 100;

		} else { //D+ : low, D- : low
			otg_io_write_with_reset(ui->xceiv, 0x25, 0x34); //Aplly current source on D+
			msleep(100);
			printk("UDC-CHG (2-2): %s (%d) : (readl(USB_PORTSC) & PORTSC_LS) = 0x%x\n", __func__, __LINE__, (readl(USB_PORTSC) & PORTSC_LS));
			...
			if ((readl(USB_PORTSC) & PORTSC_LS) & (1 << 11)) { //D+ : high 
				if ((readl(USB_PORTSC) & PORTSC_LS) & (1 << 10)) { //D+ : high, D- : high
					...
					printk("UDC-CHG (2-2-1-2): %s (%d) : Invalid Adaptor(100mA)!\n", __func__, __LINE__);
					temp = USB_CHG_TYPE__WALLCHARGER;
					atomic_set(&otg->chg_type, temp);
					maxpower = 100;
				}
			} else { //D+ : low
				otg_io_write_with_reset(ui->xceiv, 0x24, 0x34);
				msleep(10);
				if (otg_io_read_with_reset(ui->xceiv, 0x34) & (1 << 4)) {
					otg_io_write_with_reset(ui->xceiv, 0x0f, 0x34);
					writel(readl(USB_USBCMD) & ~USBCMD_RS, USB_USBCMD);
					msleep(10);
					writel(readl(USB_USBCMD) | USBCMD_RS, USB_USBCMD);
					...
					msleep(10);
					writel(readl(USB_USBCMD) | USBCMD_RS, USB_USBCMD);
					printk("UDC-CHG (2-2-2-2): %s (%d) : USB host Starndart Downstream Port(500mA)!\n", __func__, __LINE__);
					temp = USB_CHG_TYPE__SDP;
					atomic_set(&otg->chg_type, temp);
					maxpower =500 /*usb_get_max_power(ui)*/;
					if (cdev && cdev->config) {
						cdev->config->bMaxPower = 0xFA; //500mA
					}
				}
			}
		}
...
static irqreturn_t usb_interrupt(int irq, void *data)
{
	...
	if (n & STS_URI) {
		...
		wake_lock(&ui->wlock);
		if (atomic_read(&ui->configured)) {
// HP Dijack: use HP, for prevent the issue that Unplugging USB from host may cause system hang up.
#if 0
...
#if defined(CONFIG_MACH_MSM8X60_TOPAZ) || defined(CONFIG_MACH_MSM8X60_OPAL)
	//workaruond for the type of the cdev->config->bMaxPower  is u8, 700(1400/2) - 512(2^8)=188(376/2)
	if (mA == 376) {
		mA = 1400;
	}
#endif
