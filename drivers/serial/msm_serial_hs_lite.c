...
#endif
	/* HP SamLin 20110223, Add configure options for gsbi protocol and fifosize. */
	int             gsbi_protocol;
	/* End */
};
...
static void msm_hsl_set_baud_rate(struct uart_port *port, unsigned int baud)
{
    ...
    // yegw 2010-1-2
    if(port->irq == 223)  // MSM_UART10DM_PHYS for touch uart
        watermark = 4;
    // yegw end
...
static void msm_hsl_config_port(struct uart_port *port, int flags)
{
	...
	/* HP SamLin 20110223, Add configure options for gsbi protocol and fifosize. */
	{
		if ((ioread32(msm_hsl_port->mapped_gsbi + GSBI_CONTROL_ADDR) &
			msm_hsl_port->gsbi_protocol) != msm_hsl_port->gsbi_protocol)
			iowrite32(msm_hsl_port->gsbi_protocol,
...
static int __init msm_serial_hsl_probe(struct platform_device *pdev)
{
	...
	if (pdata != NULL) {
		msm_hsl_port->gsbi_protocol = *pdata;
		port->fifosize = *(pdata+1);
	} else {
		/* Set default for gsbi12.*/
		msm_hsl_port->gsbi_protocol = GSBI_PROTOCOL_I2C_UART;
	}
	/* Endif */
