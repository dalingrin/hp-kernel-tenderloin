		...
		struct sk_buff *inp, u8 code)
{
	...
	*optionp = DHCP_TAG_END;
	...
	dhcp_len = (void*)optionp - (void*)reply;
}
...
static void finish_response(struct sk_buff *response)
{
	...
	/* UDP */
//levi  Add these to do checksum for dhcp.
	memset(&packet,0,sizeof(packet));
	memcpy((void*)&(packet.udp),(void*)(udp_hdr(response)),sizeof(packet.udp));
	memcpy((void*)packet.dhcp,(void*)(bootp_hdr(response)),dhcp_len );
	packet.ip.saddr = ip_hdr(response)->saddr;
	packet.ip.daddr = ip_hdr(response)->daddr;
	packet.ip.protocol = IPPROTO_UDP;
	packet.ip.tot_len = cpu_to_be16(udplen);
	packet.udp.len = cpu_to_be16(udplen);
	packet.udp.check = 0;

	udp_hdr(response)->len = cpu_to_be16(udplen);
	udp_hdr(response)->check = finish_sum(checksum(&packet, iplen, 0));
		...
		if (!ipv4_is_multicast(ip_hdr(skb)->daddr)
		  && !is_broadcast_ether_addr((u8 *)&eth_hdr(skb)->h_dest)) {
			...
			if (SIERRA_NET_HIP_EXT_HDR_LEN <= skb_headroom(skb)) {
				...
				/* as per HIP spec we have to zero-out the Eth.
				 * header. */
				//memset(eth_hdr(skb), 0, sizeof(*eth_hdr(skb)));
