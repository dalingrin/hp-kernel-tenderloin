...
// HP Wade Wang: HW init 
#define NEED_HW_INIT
// End
//enable MPU3050_EARLY_SUSPEND_IN_DRIVER, so suspend/resume mpu could be done before native ACC driver, the process becomes: 
//suspend gyro, give control to native ACC, then suspend native ACC
...
static int mpu_open(struct inode *inode, struct file *file)
{
	...
	mpu = (struct mpu_private_data *) i2c_get_clientdata(this_client);
	mpu->mpu_is_opened = 0;
	return 0;
}
...
/*	{
	int j;
	for (j=0; j< rdwr_arg.nmsgs-1; j++)
	{
		if (rdwr_pa[j].flags==0) 
		{
			if (rdwr_pa[j+1].flags & I2C_M_RD)
			{
				res = i2c_transfer(client->adapter, &rdwr_pa[j], 2);
				if (res < 2) {
					pr_err("######### Wade: mpudev_ioctl_rdrw: i2c_transfer index:%d - read failed: res : %d\n", j, res );
					if (res>=0)
						res=-EIO;
					break;
				} 
			...
			{
				res = i2c_transfer(client->adapter, &rdwr_pa[j], 1);
				if (res < 1) {
					pr_err("######### Wade: mpudev_ioctl_rdrw: i2c_transfer index:%d - write failed: res : %d\n", j, res );
					if (res>=0)
						res=-EIO;
					break;
				} 
		...
		{
			pr_err("######### Wade: mpudev_ioctl_rdrw: index:%d - invalid parameter\n", j);
			res=-EINVAL;
			break;
		}
	}
	if (res>0 && j== rdwr_arg.nmsgs-1)
	{
		res = i2c_transfer(client->adapter, &rdwr_pa[j], 1);
	}
	if (res>0)
...
void mpu3050_early_suspend(struct early_suspend *h)
{
	...
	{
//		printk("%s: already suspened", __func__);
		return;
	}

//	printk("%s: %d, mpu->mldl_cfg.is_suspended = %d\n", __func__, h->level,
//	       mpu->mldl_cfg.is_suspended);
	...
	if (MPU3050_EARLY_SUSPEND_IN_DRIVER)
		(void) mpu3050_suspend(mldl_cfg, this_client->adapter,
				       ...
				       TRUE, TRUE);
	printk("mpu3050_early_suspend OK");
}

void mpu3050_late_resume(struct early_suspend *h)
{
	struct mpu_private_data *mpu = container_of(h,
						    struct
						    mpu_private_data,
...
//	printk("%s pid is %d, need resume = %d\n", __func__, pid, mpu->mpu_is_opened);

	if(mpu->mpu_is_opened == 0)
		return;
	accel_adapter = i2c_get_adapter(mldl_cfg->pdata->accel.adapt_num);
	compass_adapter =
	    i2c_get_adapter(mldl_cfg->pdata->compass.adapt_num);
	...
	printk("mpu3050_late_resume OK, level = %d\n", h->level);
}
#endif
...
int gyro_power(void){	
	...
	{		
		pr_err("%s: Unable to set regulator voltage:"					" votg_l15\n", __func__);
		return -1;	
	}	
	if(regulator_force_disable(votg_l15))
	{		
		pr_err("%s: Unable to disable the regulator votgl15 \n",__func__);		
		return -1;	
	}	
	if(regulator_force_disable(votg_lvs3_1v8))	
	{		
		pr_err("%s: Unable to disable the regulator votg_lvs3_1v8 \n",__func__);
		return -1;	
	}	

	pr_err("%s: Disable l15 & lvs3\n", __func__);	
	msleep(100);	
	...
	{		
		pr_err("%s: Unable to enable the regulator:"					" votg_l15\n", __func__);	
		return -1;	
	}	

	pr_err("%s: Enable l15 & lvs3\n", __func__);
	msleep(10);	
...
static struct mpu3050_platform_data s_mpu3050_data;
int mpu3050_probe(struct i2c_client *client,
		  const struct i2c_device_id *devid)
{
// HP Wade Wang: HW init 
#ifdef NEED_HW_INIT
	struct mpu3050_platform_data_ext *pdata_ext;
#endif
...
// HP Wade Wang: HW init 
#ifdef NEED_HW_INIT
	pdata_ext = (struct mpu3050_platform_data_ext *) client->dev.platform_data;
#endif
	if (!pdata) {
...
// HP Wade Wang: HW init 
#ifdef NEED_HW_INIT
		if (pdata_ext->dev_init)
		{
			printk("%s: dev_init\n", __func__);
			pdata_ext->dev_init();
		}
		memcpy(&s_mpu3050_data, pdata_ext, sizeof(s_mpu3050_data));
		mpu->mldl_cfg.pdata = &s_mpu3050_data;
#else
...
// End
	...
	res = mpu3050_open(&mpu->mldl_cfg, client->adapter);
		...
		{
			dev_err(&this_client->adapter->dev,
			"Unable to open %s for the first time: %d\n", MPU_NAME, res);
			gyro_power();
		}
	res = mpu3050_open(&mpu->mldl_cfg, client->adapter);
	...
	mpu->early_suspend.resume = mpu3050_late_resume;
...
// HP Wade Wang: HW init 
#ifdef NEED_HW_INIT
	if (pdata_ext && pdata_ext->dev_init_complete)
	{
//		printk("%s: dev_init_complete\n", __func__);
		pdata_ext->dev_init_complete();
	}
#endif
// end
	return res;
