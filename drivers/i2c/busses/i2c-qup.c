...
qup_i2c_xfer(struct i2c_adapter *adap, struct i2c_msg msgs[], int num)
{
	...
	ret = num;
 out_err:
 	/*Move disable irq before complete.Because if there is error, one cpu will
 	handle the complete, but the other one change the dev->complete. So when
 	the irq handler call the complete(dev->complete), the dev->complete could 
 	be null. It will crash the device .
 	*/
